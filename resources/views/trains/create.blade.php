...
<div class="min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-200 py-10 flex justify-center items-center">
    <div class="w-full max-w-lg bg-white rounded-xl shadow-lg p-8">
        <div class="flex flex-col items-center mb-6">
            ...
            <h1 class="text-2xl font-extrabold text-blue-700 mb-1 text-center drop-shadow">Tambah Kereta</h1>
        </div>
        <form action="{{ route('trains.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-1">Nama Kereta</label>
                <input type="text" name="name" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-200">
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-1">Tipe</label>
                <input type="text" name="type" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-200">
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 font-semibold mb-1">Jumlah Kursi</label>
                <input type="number" name="seat_count" required class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-200">
            </div>
